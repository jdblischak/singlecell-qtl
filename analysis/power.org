...
** Estimating measurement error and residual variance
  ...
  \hat\sigma^2_{mi}\) is also unbiased (Buonaccorsi 2010, Eq. 10.1).  However,
  it can happen that \(\hat\sigma^2_r \leq 0\). We need the ratio
  \(\hat\sigma^2_m / \hat\sigma^2_r\) so this strategy will not work in
  general.

  Instead, we use ~ash~ and ~vash~ ([[https://academic.oup.com/bioinformatics/article/32/22/3428/2525601][Lu and Stephens 2016]]) to solve the problem.
  ...
  \hat\sigma^2_{mi}\). However, we also need to estimate the typical
  \(\sigma^2_m\) over all genes, so we instead assume a hierarchical model on
  \(\hat\sigma^2_{mi}\).
  ...
  Although it would be ideal to jointly estimate \(g\), \(h\), we instead take
  a simpler approach:
  ...
  Compare the phenotypic variances.

  #+BEGIN_SRC ipython
    sigma2_r_disp / sigma2_r_mean
  #+END_SRC
...
** Power curves
  ...
  #+BEGIN_SRC ipython
    rel_effect = disp_effect / sigma2_r_disp
    alpha = 5e-6
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    N = st.norm()
    pow_53 = N.cdf(N.ppf(alpha / 2) + rel_effect * np.sqrt(53 / (1 + delta_disp)))
    pow_53
  #+END_SRC
  ...
  If we assume the noise ratio goes down in the number of cells, then we can
  solve analytically for the sample size \(n\) to achieve 80% power assuming 10
  times as many cells per individual

  #+BEGIN_SRC ipython
    m_80 = int(np.ceil(np.square(N.ppf(.8) - N.ppf(alpha / 2)) * (1 + delta_disp / 10) / np.square(rel_effect)))
    m_80
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/power.org/analytic-power.png
    ...
    for i, n in enumerate([53, n_80, m_80]):
      for ratio in np.linspace(0, 4, 5):
        ...
        power = N.cdf(N.ppf(alpha / 2) + grid / scale)
        ...
        if i == 2:
          realized_scale = np.sqrt((1 + delta_disp / 10) / n)
        else:
