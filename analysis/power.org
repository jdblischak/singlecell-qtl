...
** Power
   ...
   #+BEGIN_SRC ipython :ipyfile figure/power.org/lrt-power.png
     ...
     ax[1].set_ylabel('Power at level 0.05')
...
* QTL discovery
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/eqtl-power.py
    ...
    args = [(vcf, eqtls, n, m, pve)
            for n in (53, 100, 200, 400)
            for m in (1, None)
            for pve in np.linspace(0.01, 0.5, 10)]
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/power/
    sbatch --partition=broadwl -n1 -c28 --exclusive --time=4:00:00 --mem=16G --job-name=eqtl-power --output=eqtl-power.out
    #!/bin/bash
    source activate scqtl
    python /project2/mstephens/aksarkar/projects/singlecell-qtl/code/eqtl-power.py
    rsync -FFau ./ /project2/mstephens/aksarkar/projects/singlecell-qtl/data/power/
  #+END_SRC

  #+RESULTS:
  : Submitted batch job 46481028
  ...
  Look at the empirical CDF of /cis/-heritability of gene expression ([[http://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1006423][Wheeler
  et al 2016]]). The results are [[https://github.com/WheelerLab/GenArchDB][available on github]].

  #+BEGIN_SRC ipython
    import sqlite3
    with sqlite3.connect('/project2/mstephens/aksarkar/.local/src/GenArchDB/genarch.db') as conn:
      cis_h2g = pd.read_sql('select * from results where tissue == "DGN-WB";', conn)
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/power.org/eqtl-heritability.png
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    n, bins, _ = plt.hist(cis_h2g['pve'], bins=100, histtype='step', color='k', cumulative=True)
    plt.axvline(x=0.4, c='r', ls=':', lw=1)
    plt.axhline(y=n[np.where(bins >= 0.4)[0].min()], c='r', ls=':', lw=1)
  ...
  [[file:figure/power.org/eqtl-heritability.png]]
  :END:

  Estimate how many genes we had power to detect.

  #+BEGIN_SRC ipython
    m = (.95 * cis_h2g['pve'] > 0.4).sum()
    p = .7
    m * p, np.sqrt(m * p * (1 - p))
  #+END_SRC
  ...
  :RESULTS:
  # Out[22]:
  : (614.5999999999999, 13.57865972767563)
  :END:
...
* Distribution of effect sizes.
  ...
  #+BEGIN_SRC ipython
    ...
    def fit_ash(summary_stats):
      ...
      return ashr.ash(summary_stats['beta'], summary_stats['se'], method='fdr')
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/power.org/ash-fitted-g.png
    ...
    plt.plot(grid, np.array(ashr.cdf_ash(ash_results['log_phi'], grid).rx2('y')).ravel(), c='r', lw=1, label='$\hat{g}_{\mathrm{dispersion}}$')
    plt.plot(grid, np.array(ashr.cdf_ash(ash_results['log_mu'], grid).rx2('y')).ravel(), c='k', lw=1, label='$\hat{g}_{\mathrm{abundance}}$')
    plt.plot(grid, np.array(ashr.cdf_ash(ash_results['bulk'], grid).rx2('y')).ravel(), c='b', lw=1, label='$\hat{g}_{\mathrm{bulk}}$')
    plt.plot(grid, st.norm().cdf(grid), c='.75', lw=1, label='N(0, 1)')
  ...
  #+BEGIN_SRC ipython
    pd.Series({k: np.array(ashr.get_pi0(v)) for k, v in ash_results.items()})
  #+END_SRC
  ...
  # Out[71]:
  #+BEGIN_EXAMPLE
    bulk       [0.6348257407005274]
    log_mu     [0.6454298511988216]
    log_phi    [0.8401024820462243]
    dtype: object
  #+END_EXAMPLE
  :END:
...
* Distribution of PVE
  ...
  *Bayesian argument.* 
  ...
  #+BEGIN_SRC ipython :ipyfile figure/power.org/estimated-pve.png
    ...
    for k, c in zip(estimated_pve, ['k', 'r', 'b']):
