...
* Introduction
  ...
  - [[*Estimating the effect size distribution][We show that dQTL effects are larger than eQTL effects]] (in terms
    of log fold change)
  - [[*Estimating the noise ratio][We show that PVE explains lack of power to detect dQTLs]] (rather than
    measurement error)
...
*** Naive approach
    ...
    \hat\sigma^2_{mi}\) is also unbiased ([[https://www.crcpress.com/Measurement-Error-Models-Methods-and-Applications/Buonaccorsi/p/book/9781420066562][Buonaccorsi 2010]], Eq. 10.1).
...
*** Comparison of the approaches
    ...
    variances. For reference, put both the ~vash~ estimate and the naive estimate.
...
*** Per-gene approach
    ...
    The comparison above suggests that the noise ratio varies widely between
    genes. Therefore, instead of attempting to find a typical measurement error
    variance and a typical residual variance, we should keep the paired
    \(\sigma^2_{mk}, \sigma^2_{rk}\) and estimate the distribution of noise
    ratios.
    ...
    #+BEGIN_SRC ipython :ipyfile figure/power.org/noise-ratios-dist.png
    ...
    :RESULTS:
    # Out[36]:
    [[file:figure/power.org/noise-ratios-dist.png]]
    :END:

    The distributions have large outliers, so take the median as the typical
    noise ratio across genes.
    ...
    Plot the distribution of estimated residual variances. For reference,
    compare the median of the posterior means (dashed line) against the ~ash~
    prior estimate (dotted line).

    #+BEGIN_SRC ipython :ipyfile figure/power.org/sigma-rk-deconv.png
    ...
    :RESULTS:
    # Out[38]:
    [[file:figure/power.org/sigma-rk-deconv.png]]
    :END:
...
** Power curves
   ...
   #+BEGIN_SRC ipython :tangle eval :ipyfile figure/power.org/analytic-power.svg :exports none
