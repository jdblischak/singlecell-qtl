...
* Analytic power calculation
  ...
  Estimate the noise ratio for the logarithm of the mean using the delta
  method.

  #+BEGIN_SRC ipython
    get_noise_ratio(np.log(mean_by_ind), mean_sampling_var / np.square(mean_by_ind)).describe()
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/nominal-log-fc.py
    ...
    mean_by_ind = (gene_info
                   .apply(qtltools_format, axis=1)
                   .merge(log_mu, left_index=True, right_index=True))
