...
* eQTL discovery
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/eqtl-power.py
    ...
    yri = tabix.open('/scratch/midway2/aksarkar/singlecell/scqtl-mapping/yri-120-dosages.vcf.gz')
    ...
    np.random.seed(0)
    query = eqtls.sample(n=100)
    result = []
    for pve in np.geomspace(.01, 1, 10):
      for _, record in query.iterrows():
        x = read_dosage(record, yri)
        y = generate_pheno(x, pve)
        pval = permutation_test(x, y)
        result.append({'gene': record.name, 'p': x.shape[1], 'pve': pve, 'pval': pval})
    pd.DataFrame.from_dict(result).to_csv('eqtl-power.txt.gz', compression='gzip', sep='\t')
  #+END_SRC
  ...
  #+RESULTS:
  : Submitted batch job 46369383

  Read the results.

  #+BEGIN_SRC ipython
    qtl_results = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/power/eqtl-power.txt.gz', index_col=0)
    qtl_power = qtl_results.groupby('pve').apply(lambda x: (x['pval'] < 0.05).sum() / x.shape[0]).reset_index().rename(columns={0: 'power'})
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/power.org/eqtl-power.png
    plt.clf()
    plt.gcf().set_size_inches(4, 4)
    plt.plot(qtl_power['pve'], qtl_power['power'], marker='.', c='k', lw=1, ms=8)
    plt.xlabel('Proportion of variance explained')
    _ = plt.ylabel('Power at level 0.05')
  #+END_SRC
