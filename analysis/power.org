...
* QTL discovery
  ...
  Process and QC the GEUVADIS dosages.

  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/power/
    sbatch --partition=broadwl --job-name=geuvadis --out=geuvadis.out
    #!/bin/bash
    source activate scqtl
    bcftools concat -o geuvadis.vcf /project/compbio/geuvadis/genotypes/GEUVADIS.*.vcf.gz
    bgzip geuvadis.vcf
    tabix geuvadis.vcf.gz
  #+END_SRC

  #+RESULTS:
  : Submitted batch job 46400442
...
* True discovery proportion                                        :noexport:

  Power characterizes a single test; however, in a typical study we are
  performing multiple hypothesis tests and controlling the FDR.
  ...
  #+BEGIN_SRC ipython
    ...
    qvalue = rpy2.robjects.packages.importr('qvalue')

    rpy2.robjects.pandas2ri.activate()

    pd.DataFrame.from_dict({k: np.array(qvalue.pi0est(qtls[k][0]['p_beta'])[0]) for k in qtls}, orient='index').sort_values(0)
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/power.org/p-value-dist.png
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    grid = np.linspace(0, 1, 100)
    plt.hist(qtls['bulk'][0]['p_beta'].dropna(), bins=100, histtype='step', color='k', lw=1, label='Bulk')
    plt.hist(qtls['pooled'][0]['p_beta'].dropna(), bins=100, histtype='step', color='r', lw=1, label='Single cell')
  ...
  [[file:figure/power.org/p-value-dist.png]]
  :END:
