...
* Distribution of PVE
  ...
  *Bayesian argument.* 
  ...
  :END:
* Estimate variance of experimental noise
  ...
  We seek to estimate \(V[u_i]\), and how it relates to the number of cells and
  the number of molecules (sequencing depth). Importantly, we do not attempt to
  decompose it into its various components. This allows us to simply subsample
  ...
  Extract the genotypes and UMI counts for the 14 vQTLs we found.
  ...
  #+BEGIN_SRC ipython
    qtls = (pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/scqtl-mapping/variance.txt.gz', sep=' ', index_col=0)
            .sort_values('p_beta')
            .head(n=14))
  ...
  To efficiently compute the estimates, we tile the counts, and use
  onehot-encoding to get different collections of cells.

  #+BEGIN_SRC ipython
    onehot_chip_id = recode(annotations, 'chip_id')
    onehot_geno = np.concatenate([recode(geno.T, x) for x in geno.index], axis=-1)
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/tf-noise-var.py
     <<zinb-imports>>
     <<tf-imports>>
     <<power-imports>>
     <<tf-zinb-impl>>
