...
* Introduction
  ...
  - [[*Analytic power calculation][We derive the power function]] accounting for measurement error
  - [[*Estimating the effect size distribution][We show that dQTL effects are larger than eQTL effects]] (in terms
    of log fold change)
  - [[*Estimating measurement error and residual variance][We show that PVE explains lack of power to detect dQTLs]] (rather than
    measurement error)
  - [[*Power curves][We solve for the sample size]] required to achieve 80% power to detect the
    strongest dQTLs
...
* Setup                                                            :noexport:
  ...
  #+BEGIN_SRC ipython
    ...
    vashr = rpy2.robjects.packages.importr('vashr')
  #+END_SRC
...
* Analytic power calculation
  ...
  \sigma^2_m / \sigma^2_r\) as the /noise ratio/. Then, the power at level
  \(\alpha\) is:
...
** Estimating the effect size distribution
  ...
  #+BEGIN_SRC ipython :ipyfile figure/power.org/log-fc-ash-g-inset.png
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    plt.semilogx()
  ...
  :RESULTS:
  # Out[66]:
  [[file:figure/power.org/log-fc-ash-g-inset.png]]
  :END:
...
*** Naive approach
  ...
  #+BEGIN_SRC ipython
    sigma2_m_mean = log_mean_sampling_var.median(axis=1).median()
    sigma2_r_mean = (np.log(mean_by_ind).var(axis=1) - log_mean_sampling_var.median(axis=1)).median()
  ...
  #+BEGIN_SRC ipython
    sigma2_m_disp = disp_sampling_var.median(axis=1).median()
    sigma2_r_disp = (log_phi.var(axis=1) - disp_sampling_var.median(axis=1)).median()
  ...
  #+BEGIN_SRC ipython
    delta_mean = sigma2_m_mean / sigma2_r_mean
    delta_disp = sigma2_m_disp / sigma2_r_disp
...
*** Deconvolution approach
  ...
  We can also use ~ash~ and ~vash~ ([[https://academic.oup.com/bioinformatics/article/32/22/3428/2525601][Lu and Stephens 2016]]) to solve the problem.
  ...
  [[https://www.jstor.org/stable/10.2307/2965416][Cordy and Thomas 1997]] developed the same approach assuming \(\sigma^2_{mi} =
  \hat\sigma^2_{mi}\). 
  ...
  #+BEGIN_SRC ipython :async t
    sigma2_m_mean, sigma2_r_mean = estimate_variances(np.log(mean_by_ind), np.sqrt(log_mean_sampling_var))
    sigma2_m_mean, sigma2_r_mean
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :async t
    sigma2_m_disp, sigma2_r_disp = estimate_variances(log_phi, np.sqrt(disp_sampling_var))
    sigma2_m_disp, sigma2_r_disp
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    with open('deconvolution-results.pkl', 'rb') as f:
      sigma2_m_mean, sigma2_r_mean, sigma2_m_disp, sigma2_r_disp = pickle.load(f)
  #+END_SRC
...
*** Hybrid approach
  ...
  #+BEGIN_SRC ipython
    sigma2_m_mean = log_mean_sampling_var.median(axis=1).median()
    sigma2_r_mean = estimate_sigma2_r(np.log(mean_by_ind), np.sqrt(log_mean_sampling_var))
    sigma2_m_mean, sigma2_r_mean
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    sigma2_m_disp = disp_sampling_var.median(axis=1).median()
    sigma2_r_disp = estimate_sigma2_r(log_phi, np.sqrt(disp_sampling_var))
    sigma2_m_disp, sigma2_r_disp
  #+END_SRC
  ...
  To understand why the results for the residual variance are different, first
  look at the distribution of measurement error variances. For reference, put
  the `vash` estimates on this plot.

  #+BEGIN_SRC ipython :ipyfile figure/power.org/sigma-mik-hist.png
    with open('deconvolution-results.pkl', 'rb') as f:
      sigma2_m_mean, sigma2_r_mean, sigma2_m_disp, sigma2_r_disp = pickle.load(f)
    ...
    plt.axvline(x=sigma2_m_mean, lw=1, c='k', ls='--')
    ...
    plt.axvline(x=sigma2_m_disp, lw=1, c='r', ls='--')
  ...
  :RESULTS:
  # Out[42]:
  [[file:figure/power.org/sigma-mik-hist.png]]
  :END:

  Then look at the distribution of estimated per-gene measurement error
  variances. For reference, put both the `vash` estimate and the naive estimate.q

  #+BEGIN_SRC ipython :ipyfile figure/power.org/sigma-mk.png
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    plt.semilogx()
    grid = np.logspace(-2, 2, 100)
    ...
    plt.fill_between(grid, y, lw=1, color='k', alpha=0.1)
    plt.axvline(x=log_mean_sampling_var.median(axis=1).median(), lw=1, c='k', ls=':')
    plt.axvline(x=sigma2_m_mean, lw=1, c='k', ls='--')
    ...
    plt.fill_between(grid, y, lw=1, color='r', alpha=0.1)
    plt.axvline(x=disp_sampling_var.median(axis=1).median(), lw=1, c='r', ls=':')
    plt.axvline(x=sigma2_m_disp, lw=1, c='r', ls='--')
  ...
  :RESULTS:
  # Out[45]:
  [[file:figure/power.org/sigma-mk.png]]
  :END:

  Look at the distribution of naive residual variance estimates.

  #+BEGIN_SRC ipython :ipyfile figure/power.org/sigma-rk-naive.png
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    plt.xscale('symlog', linthresh=1e-2)
    grid = np.linspace(-10, 10, 100)

    sigma2_rk_mean = (np.log(mean_by_ind).var(axis=1) - log_mean_sampling_var.median(axis=1)).dropna()
    f = st.gaussian_kde(sigma2_rk_mean)
    y = f(grid)
    plt.plot(grid, y, lw=1, c='k', label='$\ln(\\mathrm{mean})$')
    plt.fill_between(grid, y, lw=1, color='k', alpha=0.1)

    sigma2_rk_disp = (log_phi.var(axis=1) - disp_sampling_var.median(axis=1)).dropna()
    f = st.gaussian_kde(sigma2_rk_disp)
    y = f(grid)
    plt.plot(grid, y, lw=1, c='r', label='$\ln(\phi)$')
    plt.fill_between(grid, y, lw=1, color='r', alpha=0.1)
  ...
  :RESULTS:
  # Out[37]:
  [[file:figure/power.org/sigma-rk-naive.png]]
  :END:

  The results suggest the fully naive version will not produce reasonable
  estimates of the typical residual variance.
...
** Dependence of measurement error on experiment size
  ...
  [[http://dx.doi.org/10.1093/biomet/37.3-4.358][Anscombe 1953]] develops the sampling theory of the NB distribution,
...
** Power curves
  ...
  #+BEGIN_SRC ipython
    rel_effect = disp_effect / np.sqrt(sigma2_r_disp)
    alpha = 5e-6
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    n_80 = int(np.ceil(np.square(N.ppf(.8) - N.ppf(alpha / 2)) * (1 + delta_disp) / np.square(rel_effect)))
    n_80
  #+END_SRC
    ...
    for i, n in enumerate([53, n_80, m_80]):
      for ratio in np.linspace(0, 4, 5):
        ...
        if i == 2:
          ...
          realized_scale = np.sqrt((1 + delta_disp) / n)
  ...
  #+BEGIN_SRC ipython :ipyfile figure/power.org/analytic-power.svg :exports none
  ...
  :RESULTS:
  # Out[98]:
  [[file:figure/power.org/analytic-power.svg]]
  :END:
