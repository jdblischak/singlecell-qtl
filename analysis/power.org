...
* Analytic power calculation
  ...
  where \(g\) is a zero-mode scale mixture of normals.

  [[https://www.jstor.org/stable/10.2307/2965416][Cordy and Thomas 1997]] developed the same approach assuming \(\sigma^2_{mi} =
  \hat\sigma^2_{mi}\) and \(g\) is a unimodal mixture of uniforms.

  However, we also need to estimate the typical \(\sigma^2_m\) over all genes,
  so we instead assume a hierarchical model on \(\hat\sigma^2_{mi}\).
  ...
  for the 95th percentile effect size relative to \(\sigma_r\), and the power
