...
* Introduction
  ...
  learning the underlying those distributions, and how to incorporate known and
  inferred confounders into that estimation procedure.
  ...
  2. [[*Zero-inflated factor analysis][We fit zero-inflated factor analysis]] and show that after explicitly
     modeling dropout, we still recover a factor correlated with sequencing
  3. [[*Effect of normalization on PCA][We show that normalization alone cannot explain the correlation we see]]
     between the top PC and sequencing
  4. [[*Effect of centering on PCA][We show that adjusting for both sample- and gene-specific means]] eliminates
     the correlation between the top PC and sequencing
...
* Effect of centering on PCA
  ...
  #+BEGIN_SRC ipython :ipyfile figure/dim-reduction.org/pca-bicentered-covars.png
    ...
    ppca_bicentered = skd.PCA(n_components=10)
    loadings_bicentered = ppca_bicentered.fit_transform(log_cpm.values.T - sample_means - feature_means.T)
    corr_bicentered = pd.concat(
      [correlation(pd.DataFrame(loadings_bicentered), cont_covars)] +
      [categorical_r2(loadings_bicentered, annotations, k) for k in cat_covars])
    corr_bicentered = corr_bicentered.pivot(index='covar', columns='pc')
    plot_pca_covar_corr(ppca_bicentered, corr_bicentered)
  #+END_SRC
  ...
  After bicentering the data, the top PC is still correlated with detection
  rate.
