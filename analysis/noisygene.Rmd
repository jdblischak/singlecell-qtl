...
```{r scde}
anno$CDKN1A <- t(eset_data_cr[row.names(eset_data_cr) == "ENSG00000124762",])
...
anno$RHOG <- t(eset_data_cr[row.names(eset_data_cr) == "ENSG00000177105",])
plot_cv_RHOG <- ggplot(anno, 
       aes(x = factor(chip_id), y = RHOG)) +
       geom_boxplot() +
  labs(x = "Individual", y = "RHOG molecule count") +
  theme(axis.text.x = element_text(hjust=1, angle = 90))
