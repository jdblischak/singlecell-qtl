...
** Call \mu-QTLs
   ...
   #+BEGIN_SRC ipython
     write_pheno_file(log_mu, gene_info, '/scratch/midway2/aksarkar/singlecell/scqtl-mapping/log-mean.bed', holdout=True)
   #+END_SRC
   ...
   #+RESULTS:
   : Submitted batch job 44976606

   Read the output.

   #+BEGIN_SRC ipython
     log_mu_qtls = read_qtltools_output('scqtl-mapping/log-mean')
   #+END_SRC
...
** Call \pi-QTLs
   ...
   #+BEGIN_SRC ipython
     write_pheno_file(logodds, gene_info, holdout=True, output_file='/scratch/midway2/aksarkar/singlecell/scqtl-mapping/logodds.bed')
   #+END_SRC
...
* QTL overlap
  ...
  #+BEGIN_SRC ipython
    ticks = ['Bulk', 'Pooled', '$\log(\mu)$', 'ZI mean']
    if 'p' in bulk_qtls:
      # Important: this collides with the field we're going to add in
      # replication_tests
      del bulk_qtls['p']
    phenos = [bulk_expr.iloc[:,5:], pooled_cpm, log_mu, mean]
    qtls = [bulk_qtls, pooled_qtls, log_mu_qtls, mean_qtls]
  ...
  #+BEGIN_EXAMPLE
    ...
    Pooled       83.2     100          100     99.2
    $\log(\mu)$  81.6      99          100      100
    ZI mean      76.2    99.1          100      100
  #+END_EXAMPLE
  :END:

  #+BEGIN_SRC ipython
    ticks = ['$\log(\mu)$', 'ZI mean', '$\log(\phi)$', 'ZI variance', 'ZI Fano']
    phenos = [log_mu, mean, log_phi, variance, fano]
    qtls = [log_mu_qtls, mean_qtls, log_phi_qtls, variance_qtls, fano_qtls]
  ...
  #+BEGIN_EXAMPLE
    ...
    $\log(\mu)$           100      100          49.5           98     80.8
    ZI mean               100      100          44.8         99.1     77.6
    $\log(\phi)$            0        0           100            0        0
    ZI variance           100      100          83.9          100      100
    ZI Fano               100     88.9           100          100      100
  #+END_EXAMPLE
  :END:
