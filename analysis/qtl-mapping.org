...
** Call \mu-QTLs
   ...
   #+BEGIN_SRC ipython
     log_mu = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/with-cell-cycle-genes/zi2-log-mu.txt.gz', index_col=0, sep=' ')
     del log_mu['NA18498']
   #+END_SRC
   ...
   From first principles, we should expect that the relative abundance is
   actually \((1 - \pi) \mu\), not \(\mu\).

   #+BEGIN_SRC ipython
     log_mu = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/with-cell-cycle-genes/zi2-log-mu.txt.gz', index_col=0, sep=' ')
     logodds = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/with-cell-cycle-genes/zi2-logodds.txt.gz', index_col=0, sep=' ')
...
** Call \phi-QTLs
   ...
   #+BEGIN_SRC ipython
     log_phi = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/with-cell-cycle-genes/zi2-log-phi.txt.gz', index_col=0, sep=' ')
     del log_phi['NA18498']
   #+END_SRC
...
** Call \pi-QTLs
   ...
   #+BEGIN_SRC ipython
     logodds = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/with-cell-cycle-genes/zi2-log-mu.txt.gz', index_col=0, sep=' ')
     del logodds['NA18498']
   #+END_SRC
   ...
   Check whether changing the algorithm produces the QTLs, or confounder
   correction does.

   #+BEGIN_SRC ipython
     logodds = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/without-confounders/zi2-log-mu.txt.gz', index_col=0, sep=' ')
     del logodds['NA18498']
   #+END_SRC
...
** Call mean-QTLs
   ...
   Estimate the corrected means.

   #+BEGIN_SRC ipython
     log_mu = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/with-cell-cycle-genes/zi2-log-mu.txt.gz', index_col=0, sep=' ')
     mean = np.exp(log_mu) * annotation.groupby('chip_id')['mol_hs'].agg(np.mean)
     del mean['NA18498']
   #+END_SRC
...
** Call variance-QTLs
   ...
   Compute the estimated corrected variances.
   ...
   #+BEGIN_SRC ipython
     ...
     mean = np.exp(log_mu) * annotation.groupby('chip_id')['mol_hs'].agg(np.mean)
     def _var(x):
       mean_by_sample = annotation[annotation['chip_id'] == x.name]['mol_hs'].values.reshape(1, -1) * x.values.reshape(-1, 1)
       var_by_sample = mean_by_sample + np.exp(log_phi)[x.name].values.reshape(-1, 1) * np.square(mean_by_sample)
       return np.mean(np.square(mean_by_sample - mean[x.name].values.reshape(-1, 1)) + var_by_sample, axis=1)
     variance = np.exp(log_mu).apply(_var, axis=0)
     del mean['NA18498']
     del variance['NA18498']
   #+END_SRC

   #+RESULTS: estimate-variance
   :RESULTS:
   # Out[204]:
   :END:
...
** Replication rates
  ...
  #+BEGIN_SRC ipython
    ...
    qtls = {pheno: (pd.read_table('{}/{}.txt.gz'.format(prefix, pheno), sep=' '),
            ...
            for pheno in ['bulk', 'pooled', 'logodds-log-mu', 'log_mu', 'log_phi', 'mean', 'variance', 'fano']}
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    ...
    covars = [pd.read_table(base / '{}-covars.txt'.format(f), sep='\s+', engine='python', index_col=0)
              for f in ('bulk', 'pooled', 'logodds-log-mu', 'log_mu', 'mean')]
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    ...
    pairwise_replication(
      qtls,
      phenos=['bulk', 'pooled', 'logodds-log-mu', 'log_mu', 'mean'],
      ticks=['Bulk', 'Pooled', '$(1 - \pi)\mu$', '$\log(\mu)$', 'ZI mean'],
      covars=covars)
  #+END_SRC
  ...
  # Out[61]:
  #+BEGIN_EXAMPLE
    Bulk  Pooled  $(1 - \pi)\mu$  $\log(\mu)$  ZI mean
    Bulk             100    82.6            35.5         82.9     82.8
    Pooled           100     100            59.8         99.2     99.2
    $(1 - \pi)\mu$   100    76.9             100         76.9     76.9
    $\log(\mu)$      100     100            65.3          100     99.6
    ZI mean          100     100            70.5          100      100
  #+END_EXAMPLE
  :END:
