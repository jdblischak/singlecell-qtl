...
** Multivariate adaptive shrinkage
   ...
   Get the index of the significant effects (lfsr < 0.01) for each gene with
   at least 50% probability of independent variance effect.

   #+BEGIN_SRC ipython
     lfsr_pass = {k: set(np.where(np.array(ashr.get_lfsr(mash_result[k])) < 0.01)[0]) for k in pihat[pihat['singletons_1'] > 0.5].index}
   #+END_SRC
