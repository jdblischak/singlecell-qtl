...
** Call \mu-QTLs
   ...
   Read the estimated parameters. The relative abundance is
...
** Call CV-QTLs
   ...
   #+BEGIN_SRC ipython
     cv = pd.DataFrame(np.sqrt(var_by_ind.T) / mean_by_ind.T, index=log_mu.index, columns=log_mu.columns)
   #+END_SRC
...
** Call Fano-QTLs
   ...
   #+BEGIN_SRC ipython
     fano = pd.DataFrame(var_by_ind.T / mean_by_ind.T, index=log_mu.index, columns=log_mu.columns)
   #+END_SRC
...
** Replication rates
  ...
  #+BEGIN_SRC ipython
    ...
    qtls = {pheno: [pd.read_table('{}/{}.txt.gz'.format(prefix, pheno), sep=' '),
            ...
            for pheno in ['bulk', 'pooled', 'log_mu', 'log_phi', 'mean', 'variance', 'cv', 'fano']}
  #+END_SRC
  ...
  The CV-QTLs are expected to have the opposite sign from the mean QTLs (under
  the Poisson, \(\mathrm{CV} \propto 1 / \sqrt(\lambda)\))
  ...
  #+BEGIN_SRC ipython
    ...
    covars = [pd.read_table(base / '{}-covars.txt'.format(f), sep='\s+', engine='python', index_col=0)
              for f in ('bulk', 'pooled', 'log_mu', 'mean', 'variance')]
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    ...
    pairwise_replication(
      qtls,
      phenos=['bulk', 'pooled', 'log_mu', 'mean'],
      ticks=['Bulk', 'Pooled', '$(1 - \pi)\mu$', 'ZI mean'],
      covars=covars[:4])
  #+END_SRC
  ...
  # Out[16]:
  #+BEGIN_EXAMPLE
    Bulk  Pooled  $(1 - \pi)\mu$  ZI mean
    Bulk             100    84.1              83     83.6
    Pooled          80.7     100            99.6     99.2
    $(1 - \pi)\mu$  81.5     100             100      100
    ZI mean         81.7     100             100      100
  #+END_EXAMPLE
  :END:
  ...
  #+BEGIN_SRC ipython
    pairwise_replication(qtls,
                         phenos=['mean', 'variance', 'cv', 'fano'],
                         ticks=['Mean', 'Variance', 'CV', 'Fano'],
                         covars=[covars[3], covars[4], None, None])
  #+END_SRC
  ...
  #+BEGIN_EXAMPLE
    ...
    Mean       100      99.3 88.9  91.8
    Variance  99.3       100 79.1   100
    CV         100      96.6  100  89.7
    Fano      96.3       100 77.8   100
  #+END_EXAMPLE
  :END:
...
** Relationship of \(p\)-values, effect sizes, and expression levels
   ...
   #+BEGIN_SRC ipython
     Xm, Ym = extract_qtl_gene_pair(variance_qtls[thresh_pass], qtls['mean'][1], dosages='/scratch/midway2/aksarkar/singlecell/scqtl-mapping/yri-120-dosages.vcf.gz')
     Cm = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/scqtl-mapping/mean-covars.txt')
   ...
   #+BEGIN_SRC ipython 
     mean_qtl_stats = qtls['mean'][0]
     mean_qtl_stats = mean_qtl_stats[mean_qtl_stats['fdr_pass']].copy()
     X, Y = extract_qtl_gene_pair(mean_qtl_stats, qtls['mean'][1], dosages='/scratch/midway2/aksarkar/singlecell/scqtl-mapping/yri-120-dosages.vcf.gz')
...
** Multivariate adaptive shrinkage                                 :noexport:
