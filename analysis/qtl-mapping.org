...
** Multivariate adaptive shrinkage
   ...
   #+BEGIN_SRC ipython
    Z = nominal_var_qtl_stats.merge(nominal_pooled_qtl_stats, on=['gene', 'id']).set_index(['gene', 'id']).filter(like='z', axis='columns')
    mash_result = {}
    for k, g in Z.groupby(level=0):
      ...
      data = mashr.mash_set_data(numpy2ri(g.values), numpy2ri(np.ones(g.shape)), V=V)
   ...
   Look at the distribution of mixture weights over all genes.
   ...
   #+BEGIN_EXAMPLE
     ...
     mean       0.367  0.000363        0.0705        0.0272          0.469
     std        0.266   0.00473         0.153        0.0912          0.288
     min       0.0124  2.44e-19             0             0              0
     25%        0.142  8.23e-11       3.1e-10      3.09e-10          0.236
     50%        0.292  8.64e-10      7.43e-09      3.11e-09          0.491
     75%        0.558  8.91e-09        0.0421      6.88e-08          0.709
     max            1    0.0734         0.825         0.701          0.988

     simple_het_1  simple_het_2  simple_het_3
     count           258           258           258
     mean       0.000135         0.002         0.064
     std         0.00217        0.0322         0.152
     min        2.44e-19      2.44e-19             0
     25%        8.46e-11      1.24e-10      2.78e-10
     50%        7.08e-10      1.09e-09      1.42e-08
     75%        7.25e-09      6.43e-09        0.0436
     max          0.0348         0.517          0.91
   #+END_EXAMPLE
   :END:

   Rank genes by posterior probability of a unique variance effect.

   #+BEGIN_SRC ipython :results output html
     print(pihat[pihat['singletons_1'] > 0.5].sort_values('singletons_1', ascending=False).merge(gene_info[['name']], left_index=True, right_index=True).to_html(classes='table'))
   #+END_SRC
   ...
   Write out the estimated posterior mixture weights.

   #+BEGIN_SRC ipython
     with sqlite3.connect('/project2/mstephens/aksarkar/projects/singlecell-qtl/browser/browser.db') as conn:
       pihat.to_sql('pihat', conn)
   #+END_SRC
   ...
   Read the estimated mixture weights.

   #+BEGIN_SRC ipython
     with sqlite3.connect('/project2/mstephens/aksarkar/projects/singlecell-qtl/browser/browser.db') as conn:
       pihat = pd.read_sql('select * from pihat', conn, index_col='index')
   #+END_SRC
   ...
   Get the index of the significant variance effects (lfsr < 0.01) for each gene with
   at least 50% probability of independent variance effect.

   #+BEGIN_SRC ipython
     lfsr_pass = {k: set(np.where(np.array(ashr.get_lfsr(mash_result[k]))[:,0] < 0.01)[0]) for k in pihat[pihat['singletons_1'] > 0.5].index}
     lfsr_pass
   #+END_SRC
   ...
   #+BEGIN_EXAMPLE
     {'ENSG00000115216': {340},
     'ENSG00000130511': {308, 416},
     'ENSG00000136111': {201, 203, 221, 222, 223, 226, 340},
     'ENSG00000137693': {435, 437, 441, 469, 471, 493, 514},
     'ENSG00000137806': set(),
     'ENSG00000160908': {20},
     'ENSG00000165724': {284, 300, 401}}
   #+END_EXAMPLE
   :END:

   Get the index of significant mean effects.

   #+BEGIN_SRC ipython
     {k: set(np.where(np.array(ashr.get_lfsr(mash_result[k]))[:,1] < 0.01)[0]) for k in pihat[pihat['singletons_1'] > 0.5].index}
   #+END_SRC
   ...
   Get the posterior mean/SD of effect sizes for the significant effects

   #+BEGIN_SRC ipython
     pm = {k: np.array(ashr.get_pm(mash_result[k]))[list(v)] for k, v in lfsr_pass.items()}
     psd = {k: np.array(ashr.get_psd(mash_result[k]))[list(v)] for k, v in lfsr_pass.items()}
   #+END_SRC
   ...
   #+BEGIN_SRC ipython
     pd.DataFrame({(k, i): x for k, v in pm.items() for i, x in enumerate(v)}).T.reset_index(level=1).merge(gene_info, left_index=True, right_index=True)[['name', 0, 1]]
   #+END_SRC
   ...
   #+BEGIN_EXAMPLE
     name         0             1
     ENSG00000115216    NRBP1 -2.427975 -2.270890e+00
     ENSG00000130511    SSBP4 -1.579683 -2.744471e-07
     ENSG00000130511    SSBP4  1.483698  1.508718e-08
     ENSG00000136111   TBC1D4  2.372016  1.864664e-02
     ENSG00000136111   TBC1D4  2.253524  4.302107e-02
     ENSG00000136111   TBC1D4  2.245763  4.341879e-02
     ENSG00000136111   TBC1D4 -2.355357 -3.712438e-02
     ENSG00000136111   TBC1D4  2.372399  1.863625e-02
     ENSG00000136111   TBC1D4  2.372399  1.863625e-02
     ENSG00000136111   TBC1D4  2.250662  7.966738e-01
     ENSG00000137693     YAP1 -2.246535 -1.635846e+00
     ENSG00000137693     YAP1 -1.645791 -2.700489e-01
     ENSG00000137693     YAP1 -2.705856 -2.509745e+00
     ENSG00000137693     YAP1 -2.704099 -2.507859e+00
     ENSG00000137693     YAP1 -2.704099 -2.507859e+00
     ENSG00000137693     YAP1 -2.707856 -2.508698e+00
     ENSG00000137693     YAP1 -2.704309 -2.507921e+00
     ENSG00000160908   ZNF394  2.408043  6.497491e-10
     ENSG00000165724  ZMYND19  1.430033  3.673143e-08
     ENSG00000165724  ZMYND19  1.430205  3.674549e-08
     ENSG00000165724  ZMYND19  1.571328  7.284712e-07
   #+END_EXAMPLE
   :END:
