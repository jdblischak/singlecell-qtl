...
* Analysis using sample moments
  ...
  We can show mathematically that defining means like this could work, but
  defining variances like this is unlikely to work. 

  Fix one individual, one gene, and let \(r\) denote the observed count. Then,
...
** Relationship of \(p\)-values, effect sizes, and expression levels
  ...
  #+BEGIN_SRC ipython 
    ...
    mean_qtl_stats = mean_qtl_stats[mean_qtl_stats['fdr_pass']]
    X, Y = extract_qtl_gene_pair(mean_qtl_stats, qtls['pooled'][1], dosages='/scratch/midway2/aksarkar/singlecell/reproduce-yang/YRI_SNPs_2_IPSC.txt.gen.gz')
    mean_qtl_stats['mean_beta'] = mean_qtl_stats['beta']
    mean_qtl_stats['mean_se'] = bootstrap_se(X, Y, Cm)
    mean_qtl_stats['mean_z'] = mean_qtl_stats['mean_beta'] / mean_qtl_stats['mean_se']
    X, Y = extract_qtl_gene_pair(mean_qtl_stats, qtls['variance'][1], dosages='/scratch/midway2/aksarkar/singlecell/reproduce-yang/YRI_SNPs_2_IPSC.txt.gen.gz')
    mean_qtl_stats['beta'] = replication_tests(X, Y, Cm).set_index('gene')['beta']
    mean_qtl_stats['se'] = bootstrap_se(X, Y, Cm)
    mean_qtl_stats['z'] = mean_qtl_stats['beta'] / mean_qtl_stats['se']
  #+END_SRC
