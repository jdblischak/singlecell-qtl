...
** Real data
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/tf-diagnostic.py
    ...
    def process_chunk(chunk, log_mu, log_phi, logodds, size, onehot):
      result = []
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation
    sbatch --partition=broadwl --job-name="tf-diagnostic" --mem=8G -n1 -c28 --exclusive --time=2:00:00
    #!/bin/bash
    source activate scqtl
  ...
  #+RESULTS:
  : Submitted batch job 55707402

  #+BEGIN_SRC sh :exports none
    sacct -j 55707402 -o Elapsed
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/design-0-ks.png
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    plt.hist(diagnostic_results['2'], density=True, color='.7', bins=10)
    plt.axhline(y=1, c='k', ls='--')
    plt.xlabel('KS test $p$-value')
    plt.ylabel('Density')
  #+END_SRC
