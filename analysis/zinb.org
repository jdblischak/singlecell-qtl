...
* Introduction
  ...
  1. [[*Tensorflow implementation][We implement GPU-based ML estimation]] of a zero-inflated negative binomial
     model
  2. [[*Simulation][We show in simulation]] that the estimates are unbiased
  3. [[*Mean expression][We compare ZINB estimates of mean expression]] against sample-based estimates
  4. [[*Evaluating goodness of fit][We implement a test for goodness of fit]] for the ZINB model
...
* Setup                                                            :noexport:
  ...
  #+CALL: ipython3(memory="16G",venv="scqtl",partition="mstephens") :dir /scratch/midway2/aksarkar/singlecell

  #+RESULTS:
  : Submitted batch job 55715182
...
** Simulation
   ...
   #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation/
     sbatch --partition=gpu2 --gres=gpu:1 --mem=16G --time=1:00:00 --job-name=tf-sim --output=tf-sim.out
     #!/bin/bash
...
** Fit ZINB2
   ...
   #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/tf-zinb.py
     ...
     log_mu, log_phi, logodds, nb_nll, zinb_nll, beta = scqtl.tf.fit(
       umi=umi,
       onehot=onehot,
       design=design,
       size_factor=size_factor,
   ...
   #+NAME: run-tf-zinb
   #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation/
     sbatch --partition=gpu --gres=gpu:1 --mem=16G --time=12:00:00 -a 0 --job-name=tf-zinb --output=zinb2.out
     #!/bin/bash
     source activate scqtl
     mkdir -p design$SLURM_ARRAY_TASK_ID
     pushd design$SLURM_ARRAY_TASK_ID
     python /project2/mstephens/aksarkar/projects/singlecell-qtl/code/tf-zinb.py --design $SLURM_ARRAY_TASK_ID
   #+END_SRC

   #+RESULTS: run-tf-zinb
   : Submitted batch job 55692205

   #+BEGIN_SRC sh :exports none
     sacct -j 55692205 -o Elapsed
   #+END_SRC
   ...
   #+NAME: rsync
   #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation/
     sbatch --partition=broadwl --time=10
     #!/bin/bash
...
** Simulation
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation/
    sbatch --partition=mstephens --mem=8G --job-name sim -n1 -c16 --time=1:00:00 --out np-sim.out
    #!/bin/bash
    source activate scqtl
...
** Fit ZINB                                                        :noexport:
  ...
  *Important notes:* 
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/shard.py
    ...
    for chunk in pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/scqtl-counts.txt.gz', index_col=0, chunksize=1000):
      ...
      if not chunk.empty:
        ...
        chunk.to_csv('/scratch/midway2/aksarkar/singlecell/density-estimation/np/chunk-{}.txt.gz'.format(i), columns=['gene', 'chip_id', 'sample', 'value', 'size'], compression='gzip', sep='\t')
        i += 1
  #+END_SRC

  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation/
    sbatch --partition=mstephens --mem=8G --job-name shard --out shard.out
    #!/bin/bash
    source activate scqtl
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/zinb.py
    ...
    with mp.Pool() as pool:
      chunk = pd.read_table('/scratch/midway2/aksarkar/singlecell/density-estimation/np/chunk-{}.txt.gz'.format(os.getenv('SLURM_ARRAY_TASK_ID')))
    ...
    with gzip.open('/scratch/midway2/aksarkar/singlecell/density-estimation/np/result-{}.txt.gz'.format(os.getenv('SLURM_ARRAY_TASK_ID')), 'wt') as f:
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation
    sbatch --partition=broadwl --job-name="np-zinb" --mem=8G --time=60 -a 21 -n1 -c28 --exclusive
    #!/bin/bash
    source activate scqtl
  ...
  #+RESULTS:
  : Submitted batch job 54910745

  #+BEGIN_SRC sh :exports none
    sacct -j 54910745 -o Elapsed
  #+END_SRC
...
* Evaluating goodness of fit
  ...
  therefore \(F\) is discontinuous. This can be done by randomizing ([[https://www.jstor.org/stable/1390802][Dunn and
  Smyth 1996]], [[https://arxiv.org/abs/1708.08527][Feng et al. 2017]]):
...
** Simulation
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/simulated-zinb.png
    n = np.exp(-res[1])
    pi0 = sp.expit(res[2])
  ...
  :RESULTS:
  # Out[25]:
  [[file:figure/zinb.org/simulated-zinb.png]]
  :END:
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/simulated-rqrs-hist.png
    Z = rpp(X[:,0], res[0], res[1], -res[2], 1e5, np.ones((X.shape[0], 1)), n_samples=50)
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
  ...
  # Out[29]:
  : Text(0,0.5,'Density')
  [[file:figure/zinb.org/simulated-rqrs-hist.png]]
  :END:
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/sim-ks.png
    tf_res = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/tf-simulation.txt.gz', index_col=0)
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
  ...
  # Out[21]:
  : Text(0,0.5,'Density')
  [[file:figure/zinb.org/sim-ks.png]]
  :END:

  Look at the distribution of test statistics.

  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/sim-ks-d.png
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    plt.hist(tf_res['ks_d'], color='.7', bins=20, density=True)
    plt.xlabel('KS $D$')
    plt.ylabel('Density')
  #+END_SRC
  ...
  # Out[24]:
  : Text(0,0.5,'Density')
  [[file:figure/zinb.org/sim-ks-d.png]]
  :END:

** Using the data twice
   :PROPERTIES:
   :CUSTOM_ID: double-use
   :END:
   ...
   #+BEGIN_SRC ipython :ipyfile figure/zinb.org/ks-test-double-use.png
     plt.clf()
   ...
   :RESULTS:
   # Out[36]:
   [[file:figure/zinb.org/ks-test-double-use.png]]
   :END:
   ...
   #+BEGIN_SRC ipython :ipyfile figure/zinb.org/ks-test-double-use-pois.png
     plt.clf()
     fig, ax = plt.subplots(1, 3, sharey=True)
     fig.set_size_inches(8, 3)
     for n, a in zip([100, 1000, 5000], ax):
   ...
   :RESULTS:
   # Out[52]:
   [[file:figure/zinb.org/ks-test-double-use-pois.png]]
   :END:

   Now fix the sample size \(n=100\) and vary the number of randomized
   quantiles drawn per observation \(m\).

   #+BEGIN_SRC ipython :ipyfile figure/zinb.org/ks-test-double-use-pois-m.png
     plt.clf()
   ...
   :RESULTS:
   # Out[59]:
   [[file:figure/zinb.org/ks-test-double-use-pois-m.png]]
   :END:
...
** Real data
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation
    sbatch --partition=broadwl --job-name="tf-diagnostic" --mem=8G -n1 -c28 --exclusive --time=60:00
    #!/bin/bash
    source activate scqtl
  ...
  #+RESULTS:
  : Submitted batch job 55713822

  Look at the histogram of KS-test p-values across all genes.

  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/design-0-ks.png
  ...
  # Out[3]:
  : Text(0,0.5,'Density')
  [[file:figure/zinb.org/design-0-ks.png]]
  :END:

  Look at randomized quantile residuals for the genes with least departure from
  ZINB.
  ...
  #+BEGIN_SRC ipython
    query = diagnostic_results.sort_values('2', ascending=False).merge(gene_info, left_index=True, right_index=True)[['name', '1', '2']].head(n=5)
    query
  #+END_SRC
  ...
  #+BEGIN_EXAMPLE
    name         1             2
    ENSG00000173818   ENDOV  0.026892  6.100999e-04
    ENSG00000196724  ZNF418  0.034138  4.320665e-06
    ENSG00000129007  CALML4  0.034768  2.657392e-06
    ENSG00000166016   ABTB2  0.039756  4.142249e-08
    ENSG00000173480  ZNF417  0.040310  2.521349e-08
  #+END_EXAMPLE
  :END:
  ...
  #+BEGIN_SRC ipython
    ...
    for chunk in pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/scqtl-counts.txt.gz', index_col=0, chunksize=100):
      chunk = chunk.loc[:,keep_samples.values.ravel()].filter(items=query.index, axis='index')
      if not chunk.empty:
        data.append(chunk)
    data = pd.concat(data)
  #+END_SRC
  ...
  Look at the histogram of randomized quantiles.

  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/hist-rqrs.png
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    plt.set_cmap('Set2')
    grid = np.linspace(0, 1, 100)
    for k, row in data.iterrows():
      Z = rpp(row, log_mu.loc[k], log_phi.loc[k], logodds.loc[k], annotations['mol_hs'], onehot, n_samples=1)
      plt.hist(Z.ravel(), histtype='step', density=True, bins=10, label=gene_info.loc[k, 'name'])
  ...
  # Out[15]:
  : Text(0,0.5,'Density')
  [[file:figure/zinb.org/hist-rqrs.png]]
  :END:

  Look at the QQ plot of the randomized quantiles.
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/rqrs.png
    ...
    plt.set_cmap('Set2')
    for k, row in data.iterrows():
      Z = rpp(row, log_mu.loc[k], log_phi.loc[k], logodds.loc[k], annotations['mol_hs'], onehot, n_samples=10)
      plt.plot(sorted(Z.ravel()), np.linspace(0, 1, np.prod(Z.shape)), label=gene_info.loc[k, 'name'])
    plt.plot([0, 1], [0, 1], c='r', ls='--')
    plt.legend(frameon=False, loc='center left', bbox_to_anchor=(1, .5))
    plt.xlabel('Theoretical quantiles')
    plt.ylabel('Observed quantiles')
  ...
  Look at one of the genes, and the estimated average density.

  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/endov.png
    k = 'ENSG00000173818'
    n = onehot.dot(np.exp(-log_phi.loc[k]))
    pi0 = onehot.dot(sp.expit(logodds.loc[k]))
    p = 1 / (1 + (annotations['mol_hs'] * onehot.dot(np.exp(log_mu.loc[k] + log_mu.loc[k]))))

    grid = np.arange(data.loc[k].max())
    pmf = []
    for value in grid:
      pmf_value = st.nbinom(n=n, p=p).pmf(value).mean()
    ...
    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    plt.hist(data.loc[k], color='k', bins=grid)
    plt.plot(.5 + grid, data.shape[1] * pmf, c='r', lw=1)
    plt.xlabel('UMI count')
  ...
  [[file:figure/zinb.org/endov.png]]
  :END:

  For comparison, look at randomized quantiles for the CPU-based solution.

  #+BEGIN_SRC ipython
    endov_res = pd.read_table('/scratch/midway2/aksarkar/singlecell/density-estimation/np/result-21.txt.gz', compression='gzip', header=None)
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/np-endov.png
    Z = rpp(data.loc['ENSG00000173818'], endov_res[2], endov_res[3], -endov_res[4], annotations['mol_hs'], onehot, n_samples=10)

    plt.clf()
    plt.gcf().set_size_inches(3, 3)
    plt.hist(Z.ravel(), density=True, bins=10)
    plt.axhline(y=1, ls=':', c='k')
    plt.xlabel('Randomized quantile')
    plt.ylabel('Density')
  #+END_SRC
  ...
  # Out[26]:
  : Text(0,0.5,'Density')
  [[file:figure/zinb.org/np-endov.png]]
  :END:

  Compare the CPU solution to the GPU solution.

  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/cpu-gpu-endov.png
    plt.clf()
    fig, ax = plt.subplots(1, 3)
    fig.set_size_inches(8, 3)
    ax[0].scatter(log_mu.loc['ENSG00000173818'], endov_res[2], c='k', s=1)
    ax[0].plot([-13,-10], [-13,-10], c='r', ls=':')
    ax[0].set_xlabel('GPU estimated $\ln\mu$')
    ax[0].set_ylabel('CPU estiamted $\ln\mu$')

    ax[1].scatter(log_phi.loc['ENSG00000173818'], endov_res[3], c='k', s=1)
    ax[1].plot([-8,2.5], [-8,2.5], c='r', ls=':')
    ax[1].set_xlabel('GPU estimated $\ln\phi$')
    ax[1].set_ylabel('CPU estiamted $\ln\phi$')

    ax[2].scatter(logodds.loc['ENSG00000173818'], endov_res[4], c='k', s=1)
    ax[2].plot([-8,1], [-8,1], c='r', ls=':')
    ax[2].set_xlabel('GPU estimated log odds')
    ax[2].set_ylabel('CPU estimated log odds')
  ...
  :RESULTS:
  # Out[19]:
  [[file:figure/zinb.org/cpu-gpu-endov.png]]
  :END:
