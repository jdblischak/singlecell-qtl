...
* numpy/scipy implementation
  ...
  #+BEGIN_SRC ipython
    ...
    def nb(theta, x, size, onehot, design=None):
      ...
      if design is not None:
        ...
        assert theta.shape == (2 * m,)
      mean = size * onehot.dot(np.exp(theta[:m]))
      if design is not None:
        mean *= np.exp(design.dot(theta[2 * m:]))
      assert mean.shape == (n,)
    ...
    def _fit_gene(chunk, onehot, design=None):
      ...
      if design is None:
        ...
        assert design.shape[0] == n
        x0 = np.zeros(2 * m + design.shape[1])
      ...
      if res0.fun < res.fun:
        ...
        # for it
        log_mu = res0.x[:m]
        neg_log_phi = res0.x[m:]
        logit_pi = np.zeros(m)
        np.fill(logit_pi, -np.inf)
      else:
        logit_pi = res.x[:m]
        log_mu = res.x[m:2 * m]
      ...
      mean_by_ind = np.ma.masked_equal(onehot * mean_by_sample.reshape(-1, 1), 0).mean(axis=0).compressed()
      var_by_ind = np.ma.masked_equal(onehot * (np.square(mean_by_sample - onehot.dot(mean_by_ind)) + var_by_sample).reshape(-1, 1), 0).mean(axis=0).compressed()
  ...
  #+BEGIN_SRC ipython
    so.minimize(_nb, np.zeros(6), (x[:,0], x[:,1], onehot, (design - design.mean(axis=0, keepdims=True))))
  #+END_SRC
  ...
  # Out[79]:
  #+BEGIN_EXAMPLE
    fun: 3.7686684938959005
    hess_inv: array([[ 3.02723427e-01,  1.22915239e-03,  1.84289227e-02,
    2.93100089e-02, -6.63234230e-03,  6.61572779e-03],
    [ 1.22915239e-03,  2.73646868e-01,  6.42487198e-03,
    -4.08994234e-02, -7.95095115e-03,  7.91377926e-03],
    [ 1.84289227e-02,  6.42487198e-03,  8.33094819e+00,
    -7.79790706e-01,  4.84845340e-03, -4.59546608e-03],
    [ 2.93100089e-02, -4.08994234e-02, -7.79790706e-01,
    1.95037075e+01, -3.76704122e-02,  3.67948000e-02],
    [-6.63234230e-03, -7.95095115e-03,  4.84845340e-03,
    -3.76704122e-02,  6.46465026e-01,  3.53473194e-01],
    [ 6.61572779e-03,  7.91377926e-03, -4.59546608e-03,
    3.67948000e-02,  3.53473194e-01,  6.46589099e-01]])
    jac: array([-4.76837158e-07,  8.34465027e-07,  1.04308128e-06,  5.66244125e-07,
    6.25848770e-07, -5.96046448e-07])
    ...
    status: 0
    success: True
    x: array([-7.79083337, -8.78481628,  2.07143452,  2.62530472,  0.16955185,
    -0.16940769])
  #+END_EXAMPLE
  :END:
  ...
  # Out[81]:
  #+BEGIN_EXAMPLE
    fun: 3.105741745890472
    hess_inv: array([[ 5.08820548e+01,  8.74220734e+00, -1.68903895e-01,
    1.66457190e-02,  2.89163844e-02,  3.40963270e-02,
    3.24767620e-01, -2.33640476e-01],
    [ 8.74220734e+00,  6.65068244e+01,  3.82097806e-01,
    -1.47080994e-01, -4.70784156e-02, -7.84243963e-02,
    -4.73011471e-01,  3.94491154e-01],
    [-1.68903895e-01,  3.82097806e-01,  1.93116093e-02,
    -8.04764776e-03, -1.35830866e-03, -5.12425983e-03,
    -4.73973226e-03,  7.88081551e-03],
    [ 1.66457190e-02, -1.47080994e-01, -8.04764776e-03,
    1.00989361e-02,  7.84247844e-04,  1.15902651e-03,
    7.16368270e-03, -7.71106773e-03],
    [ 2.89163844e-02, -4.70784156e-02, -1.35830866e-03,
    7.84247844e-04,  1.81836102e-04,  1.68098188e-04,
    1.64798846e-03, -1.36859055e-03],
    [ 3.40963270e-02, -7.84243963e-02, -5.12425983e-03,
    1.15902651e-03,  1.68098188e-04,  2.33714394e-03,
    -1.53924411e-03, -2.56078906e-04],
    [ 3.24767620e-01, -4.73011471e-01, -4.73973226e-03,
    7.16368270e-03,  1.64798846e-03, -1.53924411e-03,
    2.59333378e-02, -1.84444994e-02],
    [-2.33640476e-01,  3.94491154e-01,  7.88081551e-03,
    -7.71106773e-03, -1.36859055e-03, -2.56078906e-04,
    -1.84444994e-02,  1.45653412e-02]])
    jac: array([-1.20401382e-04, -1.87456608e-05,  9.00000334e-03, -5.87096810e-03,
    1.34749979e-01,  1.42499804e-02,  9.74801183e-03, -2.13730633e-02])
    ...
    status: 2
    success: False
    x: array([-3.08430801, -3.0794722 , -7.74672959, -8.73971707, 15.09696903,
    14.1405654 ,  0.17131679, -0.16199749])
  #+END_EXAMPLE
  :END:
  ...
  # Out[143]:
  #+BEGIN_EXAMPLE
    [array([-7.72759538, -8.72909025]),
    array([-3.54105403, -3.6325234 ]),
    array([-3.07857253, -3.07861532]),
    array([44.05020827, 16.18095952]),
    array([100.28896459,  23.10602166])]
  #+END_EXAMPLE
  :END:

  #+BEGIN_SRC ipython
    _fit_gene(x, onehot, design - design.mean(axis=0))
  #+END_SRC
  ...
  # Out[144]:
  #+BEGIN_EXAMPLE
    [array([-7.7459195 , -8.73946677]),
    array([-6.84852201, -7.45249071]),
    array([-3.07855232, -3.07858144]),
    array([43.25037785, 16.01392566]),
    array([45.23512183, 16.16266301])]
  #+END_EXAMPLE
  :END:
