...
* numpy/scipy implementation
  ...
  #+BEGIN_SRC ipython
    ...
    def nb(theta, x, size):
      """Return the per-data point log likelihood NB(x; theta, size)"""
  ...
  #+NAME: sim-impl
  #+BEGIN_SRC ipython
    def simulate(num_samples, size=None, log_mu=None, log_phi=None, logodds=None, seed=None):
      if seed is None:
        seed = 0
      np.random.seed(seed)
      if log_mu is None:
      ...
      return np.vstack((x, size * np.ones(num_samples))).T

    def evaluate(num_samples, num_mols, log_mu, log_phi, logodds, trial):
      x = simulate(num_samples=num_samples, size=num_mols,
                   log_mu=log_mu, log_phi=log_phi,
                   logodds=logodds, seed=trial)
      keys = ['num_samples', 'num_mols', 'log_mu', 'log_phi', 'logodds', 'trial',
              'log_mu_hat', 'log_phi_hat', 'logodds_hat', 'mean', 'var']
      result = [num_samples, num_mols, log_mu, log_phi, logodds, trial] + list(_fit_gene(x))
      result = {k: v for k, v in zip(keys, result)}
      # Important: follow edgeR here
      eps = .5 / num_mols
      log_cpm = np.log(x[:,0] + eps) - np.log(x[:,1] + 2 * eps) + 6 * np.log(10)
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/sim.py
    ...
    <<np-zinb-impl>>
    <<sim-impl>>
    args = [(num_samples, num_mols, log_mu, log_phi, logodds, trial)
            for num_samples in np.linspace(100, 1000, 10).astype(int)
            for num_mols in 1e3 * np.linspace(100, 1000, 10).astype(int)
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation/
    sbatch --partition=broadwl --mem=4G --job-name sim -n1 -c28 --exclusive 
    #!/bin/bash
    source activate scqtl
  ...
  *Important notes:* 
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/shard-data.py
    <<zinb-imports>>
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation/
    sbatch --partition=broadwl --mem=4G --job-name shard
    #!/bin/bash
    source activate scqtl
    python /project2/mstephens/aksarkar/projects/singlecell-qtl/code/shard-data.py
  #+END_SRC

  #+RESULTS:
  : Submitted batch job 45497237
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/zinb.py
     ...
     with mp.Pool() as pool:
       ...
       res = pool.map(_fit_gene, np.split(chunk[['value', 'size']].values, breaks[:-1]))
...
* Mean-variance relationship by individual
  ...
  #+NAME: plot-mean-var-def
  #+BEGIN_SRC ipython
    def plot_mean_var_by_gene(params, phi_hat=None, min_mean=5):
      subset = params[params['mean'] > min_mean]
      fano = subset['var'] / subset['mean']
      lim = [0, 1.05 * subset['mean'].max()]
      grid = np.linspace(*lim, num=200)
      plt.clf()
      plt.gcf().set_size_inches(4, 4)
      plt.scatter(subset['mean'], fano, s=2, c='k')
      if phi_hat is not None:
        plt.plot(grid, (grid + np.square(grid) * phi_hat) / (grid + 1e-8), c='r', ls='--', lw=1)
      else:
        for phi in np.linspace(0, 1, 5):
          plt.plot(grid, (grid + np.square(grid) * phi) / (grid + 1e-8), label='{:.2f}'.format(phi), c=colorcet.cm['inferno'](phi), ls='--', lw=1)
        plt.legend(title='Overdispersion $\phi$')    
      plt.xlim(lim)
      plt.ylim(0, 1.05 * fano.max())
      plt.xlabel('Estimated mean')
      plt.ylabel('Estimated Fano factor')

    def plot_mean_var_by_bin(params, n_bin=8, min_mean=5, equal_bins=True):
      subset = params[params['mean'] > min_mean]
      if equal_bins:
        cut = pd.qcut
        label = 'Quantile of estimated mean'
      else:
        cut = pd.cut
        label = 'Percentile of range of estimated mean'
      subset['bin'] = cut(subset['mean'], n_bin)
      ...
      for i, (k, g) in enumerate(subset.groupby('bin')):
        if len(g) < 10:
          ...
          res = plt.violinplot(g['fano'], positions=[i + .5], showextrema=True, showmedians=False, widths=.5)
          for v in res['bodies']:
            v.set_facecolor('.25')
          res['cbars'].set_color('k')
          res['cmins'].set_color('k')
          res['cmaxes'].set_color('k')
      plt.xticks(np.arange(n_bin + 1), ['{:.3f}'.format(x) for x in np.linspace(0, 1, n_bin + 1)])
      plt.xlim(0, n_bin)
      plt.xlabel(label)
      _ = plt.ylabel('Estimated Fano factor')
  #+END_SRC

  #+RESULTS: plot-mean-var-def
  :RESULTS:
  # Out[206]:
  :END:
  ...
  #+BEGIN_SRC ipython
    subset = params[params['ind'] == 'NA18507']
    subset = subset[subset['mean'] > 5]
    phi_hat = sklm.LinearRegression(fit_intercept=False).fit(subset['mean'].values.reshape(-1, 1), subset['var'] / subset['mean'] - 1).coef_
    phi_hat
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/mean-var-rel-NA18507.png
    plot_mean_var_by_gene(params[params['ind'] == 'NA18507'], phi_hat=np.array([0.17258245]))
    _ = plt.title('NA18507')
  #+END_SRC
  ...
  :RESULTS:
  # Out[207]:
  [[file:figure/zinb.org/mean-var-rel-NA18507.png]]
  :END:

  Plot the mean variance relationship by percentile (equal-sized bins).

  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/mean-var-by-percentile-NA18507.png
    plot_mean_var_by_bin(params[params['ind'] == 'NA18507'])
    plt.title('NA18507')
  #+END_SRC
  ...
  # Out[84]:
  : Text(0.5,1,'NA18507')
  [[file:figure/zinb.org/mean-var-by-percentile-NA18507.png]]
  :END:

  Plot the mean variance relationship by binning the range of mean expression
  (unequal bins)

  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/mean-var-by-quantile-NA18507.png
    plot_mean_var_by_bin(params[params['ind'] == 'NA18507'], equal_bins=False)
    plt.title('NA18507')
  #+END_SRC
  ...
  # Out[85]:
  : Text(0.5,1,'NA18507')
  [[file:figure/zinb.org/mean-var-by-quantile-NA18507.png]]
  :END:
  ...
  #+BEGIN_SRC ipython
    fano_idxmax = params.groupby('ind').agg(lambda x: x.loc[x['fano'].idxmax()]['gene'])['gene'].value_counts().to_frame()
    fano_idxmax
  #+END_SRC
  ...
  # Out[125]:
  #+BEGIN_EXAMPLE
    Chr     Start       End Strand      Name  gene
    ENSG00000110713  hs11   3692313   3692646      -     NUP98    37
    ENSG00000197061   hs6  26104104  26104518      +  HIST1H4C     1
    ENSG00000198727  hsMT     14747     15887      +    MT-CYB     1
    ENSG00000198886  hsMT     10760     12137      +    MT-ND4     7
    ENSG00000198899  hsMT      8527      9207      +   MT-ATP6     7
    ENSG00000206150  hs14  21500979  21502455      -   RNASE13     1
  #+END_EXAMPLE
  :END:
...
* Per-gene dispersion
  ...
  #+BEGIN_SRC ipython
    with sqlite3.connect('/project2/mstephens/aksarkar/projects/singlecell-qtl/browser/browser.db') as conn:
      params = pd.read_sql('select * from params;', conn)
    gene_params = params.groupby('gene').apply(lambda x: pd.Series([x['mean'].mean(), (np.square(x['mean'] - x['mean'].mean()) + x['var']).mean()])).rename(columns={0: 'mean', 1: 'var'})
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    subset = gene_params[gene_params['mean'] > 5]
    phi_hat = sklm.LinearRegression(fit_intercept=False).fit(subset['mean'].values.reshape(-1, 1), subset['var'] / subset['mean'] - 1).coef_
    phi_hat
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/overall-mean-var-rel.png
    plot_mean_var_by_gene(gene_params, phi_hat=np.array([0.24051697]))
    _ = plt.title('Average over 53 individuals')
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/overall-mean-var-rel-by-percentile.png
  ...
  :RESULTS:
  # Out[87]:
  [[file:figure/zinb.org/overall-mean-var-rel-by-percentile.png]]
  :END:
  ...
  #+BEGIN_SRC ipython :ipyfile figure/zinb.org/fano-factor-hist.png
    plt.clf()
    plt.hist(gene_params[gene_params['mean'] > 5]['fano'], color='k', bins=20)
    plt.xlabel('Estimated Fano factor')
  ...
  [[file:figure/zinb.org/fano-factor-hist.png]]
  :END:
