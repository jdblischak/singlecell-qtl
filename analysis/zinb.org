...
** Simulation
   ...
   #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-qtl/code/tf-sim.py
     ...
     res = pd.concat([evaluate(num_samples, num_mols)
                      for num_samples in (10000, 95, 1000)
   ...
   #+BEGIN_SRC ipython :ipyfile figure/zinb.org/sim1.png
     sample_pass = result['num_samples'] == 95

     plt.clf()
     fig, ax = plt.subplots(1, 3)
     fig.set_size_inches(8.5, 3)

     subset = result.loc[np.logical_and.reduce(np.vstack([sample_pass, pi_pass, fold_pass])),['log_mu', 'log_mu_hat']]
     ax[0].scatter(subset['log_mu'], subset['log_mu_hat'], s=2, c='k')
     ax[0].plot(ax[0].get_xlim(), ax[0].get_xlim(), c='r', ls=':', lw=1)
     ax[0].set_xlabel('True $\ln(\mu)$')
     ax[0].set_ylabel('Estimated $\ln(\mu)$')

     subset = result.loc[np.logical_and.reduce(np.vstack([sample_pass, mu_pass, pi_pass, fold_pass])),['log_phi', 'log_phi_hat']]
     ax[1].scatter(subset['log_phi'], subset['log_phi_hat'], s=2, c='k')
     ax[1].plot(ax[1].get_xlim(), ax[1].get_xlim(), c='r', ls=':', lw=1)
     ax[1].set_xlabel('True $\ln(\phi)$')
     ax[1].set_ylabel('Estimated $\ln(\phi)$')

     subset = result.loc[np.logical_and.reduce(np.vstack([sample_pass, fold_pass])),['logodds', 'logodds_hat']]
     ax[2].scatter(subset['logodds'], subset['logodds_hat'], s=2, c='k')
     ax[2].plot(ax[2].get_xlim(), ax[2].get_xlim(), c='r', ls=':', lw=1)
     ax[2].set_xlabel('True $\mathrm{logit}(\pi)$')
     ax[2].set_ylabel('Estimated $\mathrm{logit}(\pi)$')
   ...
   :RESULTS:
   # Out[41]:
   [[file:figure/zinb.org/sim1.png]]
   :END:

   #+BEGIN_SRC ipython :ipyfile figure/zinb.org/sim2.png
     sample_pass = result['num_samples'] == 10000

     plt.clf()
     fig, ax = plt.subplots(1, 3)
     fig.set_size_inches(8.5, 3)

     subset = result.loc[np.logical_and(sample_pass, pi_pass),['log_mu', 'log_mu_hat']]
     ax[0].scatter(subset['log_mu'], subset['log_mu_hat'], s=2, c='k')
     ax[0].plot(ax[0].get_xlim(), ax[0].get_xlim(), c='r', ls=':', lw=1)
     ax[0].set_xlabel('True $\ln(\mu)$')
     ax[0].set_ylabel('Estimated $\ln(\mu)$')

     subset = result.loc[np.logical_and.reduce(np.vstack([sample_pass, mu_pass, pi_pass])),['log_phi', 'log_phi_hat']]
     ax[1].scatter(subset['log_phi'], subset['log_phi_hat'], s=2, c='k')
     ax[1].plot(ax[1].get_xlim(), ax[1].get_xlim(), c='r', ls=':', lw=1)
     ax[1].set_xlabel('True $\ln(\phi)$')
     ax[1].set_ylabel('Estimated $\ln(\phi)$')
     ...
     ax[2].scatter(subset['logodds'], subset['logodds_hat'], s=2, c='k')
     ax[2].plot(ax[2].get_xlim(), ax[2].get_xlim(), c='r', ls=':', lw=1)
     ax[2].set_xlabel('True $\ln(\pi)$')
     ax[2].set_ylabel('Estimated $\ln(\pi)$')
...
** Fit ZINB2
   ...
   #+NAME: run-tf-zinb
   #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell/density-estimation/
     sbatch --partition=gpu --gres=gpu:1 --mem=16G --time=240 --job-name=tf-zinb --output=zinb2.out
     #!/bin/bash
     source activate scqtl
...
* Parameter distributions
  ...
  #+BEGIN_SRC ipython
    log_mu = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/design2/zi2-log-mu.txt.gz', comment='g', index_col=0, header=None, sep=' ')
    log_phi = pd.read_table('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/density-estimation/design2/zi2-log-phi.txt.gz', comment='g', index_col=0, header=None, sep=' ')
  ...
  #+BEGIN_EXAMPLE
    ...
    0
    ENSG00000000003  -9.462710 -2.643550 -7.151637
    ENSG00000000419  -9.935146 -2.985955 -6.948550
    ENSG00000000457 -12.004859 -1.351802  0.296795
    ENSG00000000460 -11.058336 -2.431379 -3.075272
    ENSG00000001036 -11.060452 -2.294582 -2.765176
  #+END_EXAMPLE
  :END:
