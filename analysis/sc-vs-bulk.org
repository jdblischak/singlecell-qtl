...
* Plot pooled subsets vs. pooled subsets
  ...
  #+BEGIN_SRC ipython
    def plot_concordance_pooled_subsets(individual, umi, annotations, ensembl, output_dir):
      umi = umi.loc[:,(annotations['chip_id'] == individual).values]
      for num_cells in [1, 10, 50, 100]:
        sample = umi.sample(n=2 * num_cells, axis=1)
        pool1 = tpm(sample.iloc[:,:num_cells].sum(axis=1).to_frame(), ensembl, log2=True)
        pool2 = tpm(sample.iloc[:,num_cells:].sum(axis=1).to_frame(), ensembl, log2=True)
