...
* Plot bulk vs. pooled single cells
  ...
  #+BEGIN_SRC ipython
    def plot_concordance(merged, title, filename, xlabel=None, ylabel=None, **kwargs):
      ...
      plt.title(title)
      if xlabel is None:
        xlabel = 'scRNA-Seq $\log_2(\mathrm{TPM} + 1) $'
      if ylabel is None:
    ...
    def plot_concordance_pooled_subsets(individual, umi, annotations, lengths, output_dir):
      ...
      for num_cells in [1, 10, 50, 100]:
        ...
        plot_concordance(
          ...
          '{}, {} cell{}'.format(individual, num_cells, 's' if num_cells > 1 else ''),
          '{}/{}-{}.png'.format(output_dir, individual, num_cells),
          ylabel='scRNA-Seq $\log_2(\mathrm{TPM} + 1)$',
          gridsize=15)
  #+END_SRC
...
* Plot bulk vs. bulk
  ...
  #+BEGIN_SRC ipython
    def plot_concordance_bulk(bulk_tpm, output_dir):
      for i in range(bulk_tpm.shape[1]):
        for j in range(i + 1, bulk_tpm.shape[1]):
          merged = bulk_tpm.iloc[:,i].to_frame().merge(
            bulk_tpm.iloc[:,j].to_frame(), left_index=True, right_index=True)
          merged.columns = ['x', 'y']
          id1 = bulk_tpm.columns[i]
          id2 = bulk_tpm.columns[j]
          plot_concordance(
            merged,
            '{} vs. {}'.format(id1, id2),
            '{}/{}-{}.png'.format(output_dir, id1, id2),
            xlabel='Bulk RNA-Seq $\log_2(\mathrm{TPM} + 1)$')
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :results none
    plot_concordance_bulk(
      bulk_tpm,
      '/home/aksarkar/projects/singlecell-qtl/analysis/figure/sc-vs-bulk.org/bulk-vs-bulk/')
  #+END_SRC

  High concordance:

  file:figure/sc-vs-bulk.org/bulk-vs-bulk/NA18499-NA19159.png

  Lower concordance:

  file:figure/sc-vs-bulk.org/bulk-vs-bulk/NA18505-NA19119.png
