...
* Plot bulk vs. single cell relative abundance
  ...
  TPM is proportional to relative abundance. *Important:* we need to quantify
  ...
  Estimate the Spearman correlation between single cell and bulk for genes
  which survive QC and have relative abundance \(< 10^{-15}\) in both data sets.

  #+BEGIN_SRC ipython
    S, T = bulk_log_rho.loc[keep_genes.values.ravel()].mask(bulk_log_rho < np.log(1e-15)).align(log_mu.loc[keep_genes.values.ravel()].mask(log_mu < np.log(1e-15)), join='inner')
    pd.Series([st.mstats.spearmanr(S[k], T[k]).correlation for k in S]).describe()
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    S, T = bulk_log_rho.align(log_mu, join='inner')
    diff = abs(S - T)
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    pd.Series((~np.isfinite(diff)).sum(axis=0) / diff.shape[0]).describe()
  #+END_SRC
  ...
  :RESULTS:
  # Out[196]:
  : array([2.22150587, 3.03904554, 4.87481989, 5.54950021, 6.94873559])
  :END:

  Look at genes only detected in one assay.

  #+BEGIN_SRC ipython
    T.mask(np.logical_or(np.isfinite(S), T < np.log(1e-15))).describe().T['count'].describe() / 20327
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    S.mask(np.logical_or(np.isfinite(T), S < np.log(1e-15))).describe().T['count'].describe() / 20327
  #+END_SRC
