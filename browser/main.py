...
def init():
  with sqlite3.connect(db) as conn:
    gene_data.data = bokeh.models.ColumnDataSource.from_df(pd.read_sql(
      ...
      from qtls where not qtls.replicated order by p_bulk;""",
      con=conn))
...
qtls = bokeh.models.widgets.DataTable(
    source=gene_data,
    columns=[bokeh.models.widgets.TableColumn(field=x, title=x) for x in ['gene', 'id', 'p_bulk', 'beta_bulk', 'p_sc', 'beta_sc']],
    width=1200,
    height=400)
...
bulk_mean_by_geno = bokeh.plotting.figure(width=400, height=400, tools=['tap', hover])
...
bulk_mean_by_geno.yaxis.axis_label = 'Quantile normalized bulk expression'
